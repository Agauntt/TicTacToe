...
if TYPE_CHECKING:
    from library.src.logic.models import Grid, GameState, Mark
    from library.src.game.players import Player

from library.src.logic.exceptions import InvalidGameState
...
def validate_starting_mark(grid:Grid, starting_mark):
...
def validate_winner(grid:Grid, starting_mark:str, winner:Mark | None) -> None:
    if winner == "X":
        if starting_mark == "X":
            if grid.x_count <= grid.o_count:
